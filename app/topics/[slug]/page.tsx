...
export async function generateMetadata(
  ...
  const description = `${topic.question} Discover what the Bible says about ${topic.title.toLowerCase()}, with verse citations and commentary from Matthew Henry.`
...
async function getTopicContent(question: string): Promise<RAGAnswer | null> {
  try {
    const { verses, chunks } = await hybridSearch(question, 5, 3)
